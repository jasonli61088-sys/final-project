...
class OnlineManager:
    ...
    def update(self, x: float, y: float, map_name: str, *, direction: str | None = None, moving: bool | None = None) -> bool:
        """Queue position update including direction and movement state."""
        if self.player_id == -1:
            return False
        try:
    ...
    async def _handle_message(self, message: str) -> None:
        ...
        try:
            ...
            if msg_type == "registered":
                ...
                with self._lock:
                    # If server sends empty chat list (after reset), clear local cache
                    if isinstance(messages, list) and len(messages) == 0 and self._last_chat_id > 0:
                        self._chat_messages.clear()
                        self._last_chat_id = 0
                        Logger.info("Chat cleared by server reset")
                    else:
                        for m in messages:
                ...
                if now - last_update >= update_interval:
                    ...
                    if latest_update and self.player_id >= 0:
                        ...
                        message = {
                            ...
                            "moving": latest_update.get("moving", False),
                        }
