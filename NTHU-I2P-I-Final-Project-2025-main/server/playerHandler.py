...
class Player:
    ...
    def update(self, x: float, y: float, map: str, direction: str, moving: bool) -> None:
        if x != self.x or y != self.y or map != self.map or direction != self.direction or moving != self.moving:
...
class PlayerHandler:
    ...
    def reset_id_counter(self) -> None:
        """Reset the ID counter to 0 when all players disconnect"""
        with self._lock:
